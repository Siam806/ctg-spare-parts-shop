 ...
 */
class ShippingAddressModuleService extends MedusaService({
  ShippingAddress,
}) {
  /**
  ...
  async getDefaultShippingAddress(customerId: string) {
    const addresses = await this.listShippingAddresses(
  ...
  async setDefaultShipping(customerId: string, addressId: string) {
    ...
    const existingDefaults = await this.listShippingAddresses({
      customer_id: customerId,
      is_default_shipping: true,
    })

    if (existingDefaults.length > 0) {
      await this.updateShippingAddresses(
    ...
    // Set the new default
    return this.updateShippingAddresses({
      id: addressId,
      is_default_shipping: true,
    })
  }
  ...
  async createCustomerAddress(data: {
    ...
    const address = await this.createShippingAddresses(data)
  ...
  async deleteCustomerAddress(addressId: string) {
    await this.deleteShippingAddresses(addressId)
  }
...
export default ShippingAddressModuleService
